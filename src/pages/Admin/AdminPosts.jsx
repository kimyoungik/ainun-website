...
export default function AdminPosts() {
  ...
  const checkAdminAndLoadPosts = async () => {
    ...
    try {
      ...
      const data = await adminService.getAllPosts(currentPage, 20);
      setPosts(data.posts);
  ...
  const handleDelete = async (postId, title) => {
    if (!confirm(`"${title}" 게시글을 삭제하시겠습니까?\n\n이 작업은 되돌릴 수 없으며, 해당 게시글의 모든 댓글과 좋아요도 함께 삭제됩니다.`)) {
      return;
    }
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b-2 border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-center text-sm font-bold text-gray-700">조회</th>
                  <th className="px-6 py-4 text-center text-sm font-bold text-gray-700">좋아요</th>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500">{post.authorGrade}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-600">{post.authorEmail}</td>
                    <td className="px-6 py-4 text-center text-gray-600">{post.viewCount}</td>
                    <td className="px-6 py-4 text-center text-gray-600">{post.likeCount}</td>
